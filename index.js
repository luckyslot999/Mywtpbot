...
// ==========================================
// 🔥 FIREBASE SESSION MANAGEMENT (100% FIXED FOR GITHUB ACTIONS)
// ==========================================
    ...
    try {
        ...
        if (data && Object.keys(data).length > 0) {
            ...
            for (const file in data) {
                // Fix: Ensure object is converted to string properly to avoid [object Object] corruption
            ...
            console.log('⚠️ No previous session found. Get ready to scan QR Code.');
        }
    } catch (error) {
...
async function uploadSession() {
    ...
    try {
        ...
        for (const file of files) {
            // Only upload JSON files to prevent garbage data
        ...
        console.log('☁️ [Firebase] Session successfully backed up!');
    } catch (error) {
        console.log('❌ Error syncing session to Firebase:', error.message);
    }
}

// ⏳ DEBOUNCER: Prevent Firebase overload (Wait 5 seconds before uploading)
let syncTimeout = null;
function debouncedUpload() {
    if (syncTimeout) clearTimeout(syncTimeout);
    syncTimeout = setTimeout(async () => {
        await uploadSession();
    }, 5000); // Waits for Baileys to completely finish writing files before uploading
}
...
async function startBot() {
    ...
    const sock = makeWASocket({
        ...
        syncFullHistory: false // Saves memory on Github Actions
    });
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            debouncedUpload(); // کنیکٹ ہونے کے بعد سیشن فائر بیس پر سیو کر دے گا
        }
        ...
        if (connection === 'close') {
            ...
            if (reason !== DisconnectReason.loggedOut) {
                ...
                // Delete session files to force fresh start next time
    ...
    sock.ev.on('creds.update', async () => {
        await saveCreds();
        debouncedUpload(); // براہ راست uploadSession کال کرنے کے بجائے delay لگا دیا گیا ہے تاکہ فائلز کرپٹ نہ ہوں
    });
